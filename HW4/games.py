...
class Hexapawn(Game):
    """Hexapawn on a 3x3 board. White ('W') moves up (+y), Black ('B') moves down (-y)."""
    ...
    def __init__(self):
        ...
        for x in range(1, 4):
            ...
            board[(x, 3)] = 'B'
        self.direction = {'W': 1, 'B': -1}
        self.home_row = {'W': 1, 'B': 3}
        self.goal_row = {'W': 3, 'B': 1}
    ...
    def compute_moves(self, board, player):
        ...
        for (x, y), piece in board.items():
            if piece != player:
                continue
            fwd = (x, y + d)
            if fwd not in board and 1 <= fwd[1] <= 3:
                moves.append(((x, y), fwd))
            for dx in (-1, 1):
                diag = (x + dx, y + d)
                if 1 <= diag[0] <= 3 and 1 <= diag[1] <= 3:
                    if board.get(diag) == opponent:
    ...
    def result(self, state, move):
        ...
        board[to] = player
        opponent = 'B' if player == 'W' else 'W'
        utility = self.compute_utility(board, to, player, opponent)
        next_moves = self.compute_moves(board, opponent) if utility == 0 else []
        return GameState(to_move=opponent, utility=utility, board=board,
                          moves=next_moves)

    def compute_utility(self, board, to_square, player, opponent):
        if to_square[1] == self.goal_row[player]:
            return 1 if player == 'W' else -1
        if not any(p == opponent for p in board.values()):
            return 1 if player == 'W' else -1
        if not self.compute_moves(board, opponent):
            return 1 if player == 'W' else -1
        return 0
...
"""Clickable Hexapawn board widget for Jupyter notebook (uses ipywidgets).

Usage in a notebook cell (after Hexapawn class and games.py are imported):

    board_widget = HexapawnWidget()
    board_widget.show()

Click a square with your pawn ('W') to select it (it will highlight),
then click an empty or enemy square to move there. After your move,
the AI (alpha_beta_player) plays automatically.
"""
